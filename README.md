...
Structured JSON logging needs no configuration, and `LOG_LEVEL` controls verbosity. Admin → Logs
reads the existing Vercel runtime logs with a server-only `VERCEL_ACCESS_TOKEN` and
`VERCEL_TEAM_ID`; Vercel supplies `VERCEL_PROJECT_ID` in hosted deployments. See
[`docs/logging.md`](./docs/logging.md) for privacy rules and setup.
...
| [docs/logging.md](./docs/logging.md)                                           | Structured logs, request correlation, admin viewer  |
